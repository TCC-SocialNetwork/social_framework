  ...
  RSpec.describe ScheduleHelper, type: :helper do
    ...
    describe "Build edges" do
      before(:each) do
        ...
        @slot1 = GraphElements::VertexDefault.new(DateTime.new(2016, 01, 01, 8, 0, 0), DateTime, {gained_weight: 0})
        @slot2 = GraphElements::VertexDefault.new(DateTime.new(2016, 01, 01, 9, 0, 0), DateTime, {gained_weight: 0})
        @slot3 = GraphElements::VertexDefault.new(DateTime.new(2016, 01, 01, 10, 0, 0), DateTime, {gained_weight: 0})
    ...
    describe "Slot empty" do
      ...
      it "When slot match event" do
        slot = GraphElements::VertexDefault.new(DateTime.new(2016, 01, 01, 10, 0, 0), DateTime)
        result = @schedule.send(:slot_empty?, slot, @event)
        expect(result).to be(false)
      end

      it "When slot not match event" do
        slot = GraphElements::VertexDefault.new(DateTime.new(2016, 01, 01, 8, 0, 0), DateTime)
        result = @schedule.send(:slot_empty?, slot, @event)
